...
namespace Oxite.Mvc
{
    public class TrackbackDiscoveryAttribute : ActionFilterAttribute
    {
        public TrackbackDiscoveryAttribute() : this(RouteTable.Routes)
        {
        }

        public TrackbackDiscoveryAttribute(RouteCollection routes)
        {
            Routes = routes;
        }
        ...
        public override void OnActionExecuted(ActionExecutedContext filterContext)
        {
            ...
            if (viewResult != null && viewResult.ViewData["Post"] != null)
            {
                ...
                if (controller.Config.Site.TrackbacksEnabled)
                {
                    ...
                    uriBuilder.Path = controller.Url.RouteUrl("Trackback", new {id = post.ID.ToString("N")});
                    viewResult.ViewData["Trackback.Url"] = uriBuilder.Uri.ToString();

                    viewResult.ViewData["Trackback.Post.Title"] = controller.PageTitle.ToString();

                    viewResult.ViewData["Trackback.Post.Url"] = post.GetAreaPostUrl(filterContext, Routes);
                }
            }
        }
    }

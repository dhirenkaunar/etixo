<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CommentFormAnonymous.ascx.cs" Inherits="OxiteSite.Views.Shared.CommentFormAnonymous" %><%
    ICommentAnonymous commentAnonymous = ViewData.Model as ICommentAnonymous; %>
    <form method="post" id="comment" action="<%=Url.Post((IPost)ViewData["Post"]) %>#comment">
        <fieldset class="info">
            ...
            <div class="remember">
                <%= Html.CheckBox("comment_remember", Request.Form.IsTrue("comment_remember") || (commentAnonymous != null && !string.IsNullOrEmpty(commentAnonymous.HashedEmail)), new { tabindex = "5" })%>
                <label for="comment_remember"><%= Localize("Remember your info?") %></label>
            </div>
            <div class="subscribe">
                <%= Html.CheckBox("comment_subscribe", Request.Form.IsTrue("comment_subscribe"), new { tabindex = "6" })%>

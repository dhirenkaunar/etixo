<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CommentFormAuthenticated.ascx.cs" Inherits="OxiteSite.Views.Shared.CommentFormAuthenticated" %><% 
    IUser user = (IUser)ViewData.Model; %>
    <form method="post" id="comment" action="<%=Url.Post((IPost)ViewData["Post"]) %>#comment" class="user">
        <div class="avatar"><%= Html.Gravatar(user, "48", Config.Site.GravatarDefault) %></div>
